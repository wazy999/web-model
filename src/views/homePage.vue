<template>
  <div style="background:#1a1a1a;color:white;height:100vh;">
    ...
    <el-row :gutter="5" class="content">
      <el-col :span="5">
        <div class="iconfont icon-tianqi-" style="font-size:80px"></div>
        <span style="font-size:40px;color: gray;">{{weatherinfo.temp}}℃</span>
      </el-col>
      <el-col :span="5">
        <div class="iconfont icon-shidu" style="font-size:80px"></div>
        <span class="param">
          相对湿度
          <br />
          {{weatherinfo.SD}}
      ...
      <el-col :span="6">
        <div class="iconfont icon-qiya" style="font-size:80px"></div>
        <span class="param">
          气压
          <br />
          {{weatherinfo.AP}}
      ...
      <el-col :span="5">
        <div class="iconfont icon-fengsu" style="font-size:80px"></div>
        <span class="param">
          {{weatherinfo.WD}}
          <br />
          {{weatherinfo.sm}}级
    ...
    <el-row class="roomCard" :gutter="15">
      <el-col :span="6" v-for="(item,id) in roomList" :key="id" style="height:180px;margin:10px 0">
        <div class="smallCard" @click="tapEquip(item.roomName)">
          <img
            src="../assets/image/jiadian.png"
            style="width:70px;height:70px;position:absolute;left:8%;top:30%"
            mode
          />
...
export default {
  ...
  methods: {
    tapEquip(e) {
      this.$router.push({ path: "/equipment", query: { roomName: e } });
    },
    getWeather() {
      this.$axios
